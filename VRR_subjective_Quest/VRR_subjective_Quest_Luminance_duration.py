...
def vrr_one_block(glfw, window, vrr_params, signal_params, random_vrr_period, c_params):
    x_center, y_center, x_scale, y_scale, interval_time, vrr_color, duration = c_params
    ...
    while not glfw.window_should_close(window):
        ...
        if display_t < signal_params['signal_time']: #第一个红色signal
        ...
        elif display_t < signal_params['signal_time'] + duration: #第一个展示片段
            color = vrr_color
            if random_vrr_period == 0: #VRR
                ...
                frame_rate = vrr_params['fix_frame_rate']
        elif display_t < signal_params['signal_time'] * 2 + duration: #第二个signal
            color = signal_params['signal_2_color']
            frame_rate = vrr_params['fix_frame_rate']
        elif display_t < signal_params['signal_time'] * 2 + duration * 2: #第二个展示片段
            color = vrr_color
...
def vrr_exp_main(change_parameters, vrr_params, signal_params, save_path, MOA_save_path, random_shuffle):
    ...
    experiment_record = {'Block_ID': [], 'VRR_Frequency': [], 'Size_Degree': [], 'Duration': [],
                         'Threshold_Color_Value': [], 'Trail_ID': [], 'Real_VRR_period': [],
                         'Observer_choice': [], 'Response': []} # Real_VRR_period == Observer_choice --> Response = 1
    ...
    for vrr_f in change_parameters['VRR_Frequency']:
        for size in change_parameters['Size']:
            for duration in change_parameters['Duration']:
                setting_params = vrr_f, size, duration
    ...
    while index < len(setting_list):
        ...
        vrr_f, size, duration  = setting_params
        print('VRR_Frequency', vrr_f, 'Size', size, 'Duration', duration)
        MOA_result = np.array(MOA_result_dict[f'V_{vrr_f}_S_{size}'])
        MOA_mean = MOA_result.mean()
        MOA_std = 0.015380745877557448 * change_parameters['STD_multiple']
        ...
        for quest_trail_index in range(change_parameters['Trail_Number']):  # 这里的50是你想运行Quest的次数
            ...
            c_params = x_center, y_center, x_scale, y_scale, interval_time, vrr_color, duration
            ...
            #         experiment_record[key].pop()
                ...
                experiment_record['Duration'].append(duration)
        ...
        final_result_json_dict[f'V_{vrr_f}_S_{size}_D_{duration}'] = {
...
if __name__ == "__main__":
    change_parameters = {
        ...
        'Size': [16],
        'Duration': [0.1, 0.2, 0.4, 0.8, 1.6, 3.2],
        'Trail_Number': 30,
        'STD_multiple': 1,
        'power_scale': 2.2,
    }
    ...
    save_base_path = r'Result_Quest_duration_1/'
    save_path = os.path.join(save_base_path, f"Observer_{observer_params['name']}")
    MOA_save_path = os.path.join(r'../VRR_Subjective_MOA/Result_MOA_3/', f"Observer_{observer_params['name']}", 'result.json')
