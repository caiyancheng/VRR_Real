...
Quest_exp_path = r'..\VRR_subjective_Quest\Result_Quest_gabor_1\Observer_Yancheng_Cai_2'
...
Quest_Gabor_Fs = Quest_config['change_parameters']['Gabor_Frequency']
...
for vrr_f_index in range(len(Quest_VRR_Fs)):
    ...
    for size_index in range(len(Quest_Sizes)):
        ...
        for gabor_f_index in range(len(Quest_Gabor_Fs)):
            gabor_f_value = Quest_Gabor_Fs[gabor_f_index]
            for trail_id in range(Trail_number):
                subset = df[(df['VRR_Frequency'] == vrr_f_value) & (df['Size_Degree'] == size_value)
                            & (df['Gabor_Frequency'] == gabor_f_value) & (df['Trail_ID'] == trail_id)]
                for key in original_keys:
