...
def vrr_exp_main(change_parameters, vrr_params, signal_params, save_path, MOA_save_path, random_shuffle, continue_exp):
    ...
    if continue_exp:
        ...
        experiment_record = {'Block_ID': [], 'VRR_Frequency': [], 'Size_Degree': [], 'Threshold_Color_Value': [],
                             'Trail_ID': [], 'Real_VRR_period': [], 'Observer_choice': [], 'Eccentricity': [],
    ...
    while index < len(setting_list):
        ...
        vrr_f, size, ecc = setting_params
        print('VRR_Frequency', vrr_f, 'Size', size, 'Eccentricity', ecc)
        if continue_exp and f'V_{vrr_f}_S_{size}_E_{ecc}' in final_result_json_dict.keys():
            print('Jump')
            index = index + 1
            continue
        ...
        MOA_result = np.array(MOA_result_dict[f'V_{vrr_f}_S_{size}_E_{ecc}'])
        ...
        final_result_json_dict[f'V_{vrr_f}_S_{size}_E_{ecc}'] = {
...
if __name__ == "__main__":
    ...
    save_base_path = r'Result_Quest_disk_eccentricity_1/'
    save_path = os.path.join(save_base_path, f"Observer_{observer_params['name']}")
    MOA_save_path = os.path.join(r'../VRR_Subjective_MOA/Result_MOA_disk_eccentricity_1/', f"Observer_{observer_params['name']}", 'result.json')
