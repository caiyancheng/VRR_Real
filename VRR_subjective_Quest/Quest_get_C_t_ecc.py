...
Quest_Eccentricitys = Quest_config['change_parameters']['Eccentricity']

C_t_result_csv = {}
C_t_result_csv['Size_Degree'] = []
C_t_result_csv['VRR_Frequency'] = []
C_t_result_csv['Eccentricity'] = []
...
for vrr_f_index in range(len(Quest_VRR_Fs)):
    ...
    for size_index in range(len(Quest_Sizes)):
        ...
        for ecc_index in range(len(Quest_Eccentricitys)):
            ecc_value = Quest_Eccentricitys[ecc_index]
            filtered_df = df[(df['Size_Degree'] == size_value) & (df['VRR_Frequency'] == vrr_f_value)
                             & (df['Eccentricity'] == ecc_value)]
            ...
            try:
                ...
                coefficients = fit_set['size_16']['coefficients']
                popt = fit_result['size_all']['popt']
            if np.isnan(Color_value):
                continue
            if size_value != 'full':
            ...
            C_t_result_csv['Eccentricity'].append(ecc_value)
