...
def vrr_one_block_square(glfw, window, vrr_params, signal_params, random_vrr_period, c_params):
    global screen_width, screen_height
    x_center, y_center, x_scale, y_scale, interval_time, vrr_color, f = c_params
    ...
    while not glfw.window_should_close(window):
        ...
        if glfw.get_key(window, glfw.KEY_ESCAPE) == glfw.PRESS:
            return -1
        glColor3f(vrr_color[0]*2, vrr_color[1]*2, vrr_color[2]*2)
        glBegin(GL_QUADS)
        glTexCoord2f(0, 0)
        glVertex2f(-1, -1)
        glTexCoord2f(1, 0)
...
def vrr_exp_main(change_parameters, vrr_params, signal_params, save_path, MOA_save_path, random_shuffle, continue_exp):
    ...
    if continue_exp:
        ...
        experiment_record = {'Block_ID': [], 'VRR_Frequency': [], 'Size_Degree': [], 'Gabor_Frequency':[],
    ...
    while index < len(setting_list):
        ...
        print('VRR_Frequency', vrr_f, 'Size', size, 'Gabor_Frequency', gabor_f)
        # if continue_exp and f'V_{vrr_f}_S_{size}_G_{gabor_f}' in final_result_json_dict.keys():
        #     print('Jump')
        #     index = index + 1
        #     continue
        ...
        MOA_result = np.array(MOA_result_dict[f'V_{vrr_f}_S_{size}_G_{gabor_f}'])
        ...
        for quest_trail_index in range(change_parameters['Trail_Number']):  # 这里的50是你想运行Quest的次数
            ...
            c_params = x_center, y_center, x_scale, y_scale, interval_time, vrr_color, gabor_f
            if size == 'full':
...
if __name__ == "__main__":
    ...
    save_base_path = r'Result_Quest_gabor_1/'
    save_path = os.path.join(save_base_path, f"Observer_{observer_params['name']}")
    MOA_save_path = os.path.join(r'../VRR_Subjective_MOA/Result_MOA_gabor_1/', f"Observer_{observer_params['name']}", 'result.json')
