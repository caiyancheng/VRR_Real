...
for observer_path in observer_path_list:
    ...
    if observer_path not in observer_name_list:
        continue
    Quest_file_path = os.path.join(Quest_root_path, observer_path, 'matlab_reorder_result_D_thr_C_t.csv')
...
df.to_csv(f'{Quest_root_path}/Matlab_D_thr_C_t_gather.csv', index=False)