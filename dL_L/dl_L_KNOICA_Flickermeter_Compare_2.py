...
root_path=r'B:\Datasets\Temporal_Flicker_Meter_log\deltaL_L\2023-12-17-05-06-02'
with open(os.path.join(root_path, 'config.json'), 'r') as fp:
    config_data = json.load(fp)
with open(r'B:\Py_codes\VRR_Real\dL_L\Temporal_Results/dl_L_results.json', 'r') as fp:
...
for size_index in range(len(Size_list)):
    ...
    current_dl_array = y_dl_array[size_index, :, :].mean(axis=0)
    current_dl_L_array = y_dl_L_array[size_index, :, :].mean(axis=0)
