...
color_plot = ['r', 'b']
x_ecc_fit_plot = np.linspace(s_ecc_list[0], s_ecc_list[-1], 100)
            ...
            for s_pers_index in range(len(s_pers_list)):
                ...
                for c_size_index in range(len(c_size_list)):
                    ...
                    for c_color_index in range(len(c_color_list)):
                        ...
                        coefficients = np.polyfit(x_ecc_list, y_CFF_list_ecc, degree)
                        y_CFF_fitted_curve = np.polyval(coefficients, x_ecc_fit_plot)
                        plt.subplot(1, 2, s_size_index + 1)
                        plt.plot(x_ecc_fit_plot, y_CFF_fitted_curve, color=color_plot[s_size_index])
