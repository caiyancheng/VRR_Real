...
plt.subplot(1, 2, 2)
for plot_index in range(len(x_Time_array_plot_list)):
    plt.plot(x_Frequency_plot_list[plot_index][1:], y_K_FFT_plot_list[plot_index][1:], linewidth=0.8)
plt.xlabel('Temporal Frequency (Hz)')
plt.ylabel('Amplitude')
plt.title('Fourier Transform (0 Hz is skipped)')
# plt.grid(True)
plt.xticks([aim_vrr_f, 30, 120])  # 设置横轴刻度
plt.axvline(x=aim_vrr_f, color='y', linestyle='--', label=f'{aim_vrr_f} Hz (Flicker)', linewidth=0.5)  # 添加竖线
...
plt.legend()
plt.subplots_adjust(left=0.06, right=0.99, top=0.95, bottom=0.10, wspace=0.18)
# plt.show()
plt.savefig(f'B:\All_Conference_Papers\SIGGRAPH24\Images/FFT_{aim_vrr_f}.png', dpi=300)