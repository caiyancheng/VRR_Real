...
fit_config.csf_models = {CSF_lTCSF_3()};
...
fitpars_dir = "E:\Matlab_codes\csf_datasets\model_fitting\fitted_models\csf-cyc-2024-4-26-lTCSF3-yancheng_1";
initial_k_scale_s = ones(N_models,1)*0.1926; %S_new = S / k
...
% 是否需要fit k_values参数
optimized_k_scale_s = ones(N_models,1)*0.1; %S_new = S / k
...
if (optimize_need == 1)
    ...
    for model_index=1:N_models
        ...
        loss_func_fit = @(k_scale) VRR_CSF_loss_func(csf_model_use, size_indices, FRR_indices, average_S_matrix, k_scale, Sensitivity_transform);
    ...
    writematrix(optimized_k_scale_s, 'fit_result/VRR_Flicker_lTCSF/optimized_k_scale_s_VRR_lTCSF3.csv');
    writematrix(fvals, 'fit_result/VRR_Flicker_lTCSF/fvals_VRR_lTCSF3.csv');
else
    optimized_k_scale_s = readmatrix('fit_result/VRR_Flicker_lTCSF/optimized_k_scale_s_VRR_lTCSF3.csv');
end
    ...
    for size_i = 1:length(size_indices)
        ...
        for FRR_range_i = 1:length(FRR_range)
            ...
            for model_index = 1:N_models
                ...
                [L_thr, S_thr_csf, S_thr_transform] = VRR_find_aim_Luminance(csf_model_use, FRR_range_value, radius, optimized_k_scale_s(model_index), Sensitivity_transform);
                S_results_x_FRR_range(model_index, FRR_range_i, size_i) = S_thr_csf;
            end
        end
    end
    writematrix(S_results_x_FRR_range, 'fit_result/VRR_Flicker_lTCSF/S_lTCSF3_results_x_FRR_range');
else
    S_results_x_FRR_range_flat = readmatrix('fit_result/VRR_Flicker_lTCSF/S_lTCSF3_results_x_FRR_range');
...
if plot_FRR == 1
    ...
    for size_i = 1:length(size_indices)
        ...
        if (size_value == -1)
            ...
            display_name_predict = 'lTCSF3 Prediction - Size: 62.7^{\circ}*37.8^{\circ}';
        else
            area_value = pi*size_value^2;
            radius = size_value;
            display_name_gt = ['Subjective Result - Size: disk radius ' num2str(size_value/2) '^{\circ}'];
            display_name_predict = ['lTCSF3 Prediction - Size: disk radius ' num2str(size_value/2) '^{\circ}'];
        end
        hold on;
        % set(gca, 'XScale', 'log');
        set(gca, 'YScale', 'log');
