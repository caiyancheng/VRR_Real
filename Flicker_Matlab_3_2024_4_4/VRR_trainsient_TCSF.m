...
transient_TCSF_params = [8, 0.1898, 0.12314];
...
S_results_transient_fit = zeros(length(FRR_indices), length(size_indices));
S_results_transient_plot = zeros(length(FRR_range), length(size_indices));
...
for size_i = 1:length(size_indices)
    ...
    for FRR_i = 1:length(FRR_indices)
        ...
        S_results_transient_fit(FRR_i,size_i) = S_TCSF_2(transient_TCSF_params, FRR_value);
    end
end

%拟合阶段
options = optimset('Display', 'iter');
loss_function_transient = @(k_transient) loss_transient(k_transient, size_indices, FRR_indices, average_S_matrix, S_results_transient_fit);
lb = 1e-5;
ub = Inf;
[optimized_k_transient, fval] = fmincon(loss_function_transient, initial_k, [], [], [], [], lb, ub, [], options);

% optimized_k_transient = 1;
%正式运算阶段
for size_i = 1:length(size_indices)
    for FRR_i = 1:length(FRR_range)
        FRR_value = FRR_range(FRR_i);
        S_results_transient_plot(FRR_i,size_i) = S_TCSF_2(transient_TCSF_params, FRR_value)./optimized_k_transient;
    end
end
%绘图阶段
...
%  S_results_transient_plot = log(S_results_transient_plot);
...
hh(end+1) = plot(FRR_range, S_results_transient_plot(:,1), '-', 'LineWidth', 3, 'Color', 'r', 'DisplayName', 'Transient TCSF Sensitivity Prediciton');
hLegend = legend(hh,'FontSize',9);

function [loss] = loss_transient(k_transient, size_indices, FRR_indices, average_S_matrix, S_results_transient_fit)
    loss = 0;
    for size_i = 1:length(size_indices)
        for FRR_i = 1:length(FRR_indices)
            loss = loss + (log10(S_results_transient_fit(FRR_i,size_i)/k_transient)-log10(average_S_matrix(FRR_i,size_i)))^2;
        end
    end
end