...
if (generate_surface == 1)
    Sensitivity_ecc_temporal_1 = zeros(length(Area_plot_list), length(Temporal_Frequency_list));
    Area_surface_matrix_1 = zeros(length(Area_plot_list), length(Temporal_Frequency_list));
    TF_surface_matrix_1 = zeros(length(Area_plot_list), length(Temporal_Frequency_list));
    for Area_index = 1:length(Area_plot_list)
        ...
        for tf_index = 1:length(Temporal_Frequency_list)
            ...
            Area_surface_matrix_1(Area_index, tf_index) = Area_value;
            TF_surface_matrix_1(Area_index, tf_index) = tf_value;
            csf_pars = struct('s_frequency', 0, 't_frequency', tf_value, 'orientation', 0, ...
                'luminance', default_lum, 'area', Area_value, 'eccentricity', 0);
            Sensitivity_ecc_temporal_1(Area_index, tf_index) = csf_elaTCSF_model.sensitivity(csf_pars);
        end
    end
    writematrix(Sensitivity_ecc_temporal_1, 'first_figure/Sensitivity_ecc_temporal_1');
    writematrix(Area_surface_matrix_1, 'first_figure/Area_surface_matrix_1');
    ...
    Sensitivity_ecc_temporal_1 = reshape(Sensitivity_ecc_temporal_1_flat, [length(Area_plot_list), length(Temporal_Frequency_list)]);
    Area_surface_matrix_1_flat = readmatrix('first_figure/Area_surface_matrix_1');
    Area_surface_matrix_1 = reshape(Ecc_surface_matrix_1_flat, [length(Area_plot_list), length(Temporal_Frequency_list)]);
    TF_surface_matrix_1_flat = readmatrix('first_figure/TF_surface_matrix_1');
    TF_surface_matrix_1 = reshape(TF_surface_matrix_1_flat, [length(Area_plot_list), length(Temporal_Frequency_list)]);
...
if (plot_surface==1)
    ...
    axes(ha(1));
    surf(Area_surface_matrix_1, TF_surface_matrix_1, log10(Sensitivity_ecc_temporal_1), 'EdgeColor','none', 'FaceAlpha', 1);
    hold on;
    colormap(flipud(hsv));
    xlabel('Area (degree^2)', FontSize=14);
