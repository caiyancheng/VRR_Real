function loss_sum = peak_fit_loss_all(csf_model, size_indices, vrr_f_indices, average_C_t_matrix, k_scale, fit_poly_degree, Luminance_lb, Luminance_ub, peak_spatial_frequency)
    ...
    for size_i = 1:length(size_indices)
        ...
        for vrr_f_i = 1:length(vrr_f_indices)
            ...
            [L_thr_predict_results_energy_fit(vrr_f_i, size_i), C_thr_predict_results_energy_fit(vrr_f_i, size_i)] = peak_generate_contrast_all(csf_model, vrr_f_value, area_value, radius, k_scale, fit_poly_degree, Luminance_lb, Luminance_ub, peak_spatial_frequency);
