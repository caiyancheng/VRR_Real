...
suffixes = {'stelaCSF (Simple)', 'stelaCSF_{HF} (Simple)', 'BartenCSF (Simple)', 'BartenCSF_{HF} (Simple)', ...
    'castleCSF (Simple)', 'stelaCSF transient (Simple)', 'stelaCSF_{HF} transient (Simple)'};
...
if optimize_need == 1
    ...
    initial_setting_luminance_value = average_C_t_matrix(initial_setting_vrr_f_index, initial_setting_radius_index);
    for csf_model_index = 1:length(CSF_Model_cell)
        csf_model_use = CSF_Model_cell{csf_model_index};
        csf_pars = struct('s_frequency', initial_s_frequency_values(csf_model_index), 't_frequency', initial_setting_vrr_f_value, 'orientation', 0, ...
            'luminance', initial_setting_luminance_value, 'area', initial_setting_area_value, 'eccentricity', 0);
        S_simple = csf_model_use.sensitivity(csf_pars);
        C_thr_sub = average_C_t_matrix(initial_setting_vrr_f_index, initial_setting_radius_index);
        initial_k_scale_values(csf_model_index) = 1 /  (S_simple * C_thr_sub);
    end
end
...
optimized_s_frequency_values = zeros(length(suffixes), 1);
...
if optimize_need == 1
    ...
    for csf_model_index = 1:length(CSF_Model_cell)
        csf_model_use = CSF_Model_cell{csf_model_index};
        objective_function = @(params) simple_fit_loss_all(csf_model_use, size_indices, vrr_f_indices, average_C_t_matrix, ...
            params(1), params(2), fit_poly_degree, Luminance_lb, Luminance_ub).*loss_multiple_factor;
        initial_params = [initial_k_scale_values(csf_model_index), initial_s_frequency_values(csf_model_index)];
        loss_initial = objective_function(initial_params);
        [optimized_params, fval] = fmincon(@(params) objective_function(params), initial_params, [], [], [], [], lb, ub, [], options);
        optimized_k_scale_values(csf_model_index) = optimized_params(1);
        optimized_s_frequency_values(csf_model_index) = optimized_params(2);
        fvals(csf_model_index) = fval./loss_multiple_factor;
    end
    writematrix(optimized_k_scale_values, 'optimized_k_scale_values_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
    writematrix(optimized_s_frequency_values, 'optimized_s_frequency_values_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
    writematrix(fvals, 'fvals_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
else
    optimized_k_scale_values = readmatrix('optimized_k_scale_values_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
    optimized_s_frequency_values = readmatrix('optimized_s_frequency_values_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
end
        ...
        for vrr_f_range_i = 1:length(continuous_vrr_f_range)
            ...
            for csf_model_index = 1:length(CSF_Model_cell)
                ...
                [~,C_thr_results_x_vrr_f_range(csf_model_index,vrr_f_range_i,size_i)] = ...
                    simple_generate_contrast_all(csf_model_use, area_value, vrr_f_range_value, optimized_k_scale_values(csf_model_index), ...
                    optimized_s_frequency_values(csf_model_index), fit_poly_degree, Luminance_lb, Luminance_ub);
            end
        end
    end
    writematrix(C_thr_results_x_vrr_f_range, 'C_thr_results_range_x_vrr_f_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
else
    C_thr_results_x_vrr_f_range_flat = readmatrix('C_thr_results_range_x_vrr_f_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
...
if (csv_generate_area == 1)
    ...
    writematrix(C_thr_results_x_area_range, 'C_thr_results_range_x_area_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
else
    C_thr_results_x_area_range_flat = readmatrix('C_thr_results_range_x_area_final_Quest_disk_C_t_simple_eachone_plot_1.csv');
