...
suffixes = {'stelaCSF (Original Energy)', 'stelaCSF_{HF} (Original Energy)',  'BartenCSF (Original Energy)', 'BartenCSF_{HF} (Original Energy)', ...
            ...
            'stelaCSF (Fixed Area + SI power 0.5)', 'stelaCSF_{HF} (Fixed Area + SI power 0.5)',  'BartenCSF (Fixed Area + SI power 0.5)', 'BartenCSF_{HF} (Fixed Area + SI power 0.5)', ...
            'castleCSF (Fixed Area + SI power 0.5)', 'stelaCSF transient (Fixed Area + SI power 0.5)', 'stelaCSF_{HF} transient (Fixed Area + SI power 0.5)', ...
            'stelaCSF (Fixed Area + radius power 2)', 'stelaCSF_{HF} (Fixed Area + radius power 2)',  'BartenCSF (Fixed Area + radius power 2)', 'BartenCSF_{HF} (Fixed Area + radius power 2)', ...
            'castleCSF (Fixed Area + radius power 2)', 'stelaCSF transient (Fixed Area + radius power 2)', 'stelaCSF_{HF} transient (Fixed Area + radius power 2)', ...
            'stelaCSF (Fixed Area + eccentricity power 2)', 'stelaCSF_{HF} (Fixed Area + eccentricity power 2)',  'BartenCSF (Fixed Area + eccentricity power 2)', 'BartenCSF_{HF} (Fixed Area + eccentricity power 2)', ...
            'castleCSF (Fixed Area + eccentricity power 2)', 'stelaCSF transient (Fixed Area + eccentricity power 2)', 'stelaCSF_{HF} transient (Fixed Area + eccentricity power 2)'};
...
Energy_Model_cell = {@energy_model_original, @energy_model_fixarea_beta, energy_model_spatial_fixarea_pow05, energy_model_spatial_fixarea_radius_pow2, energy_model_spatial_fixarea_ecc_pow2};
...
if (optimize_need == 1)
    ...
    writematrix(optimized_E_thr_values, 'optimized_E_thr_values_final_Quest_disk_C_t_energy_eachone_plot_3.csv');
    writematrix(fvals, 'fvals_x_final_Quest_disk_C_t_energy_eachone_plot_2.csv');
else
    optimized_E_thr_values = readmatrix('optimized_E_thr_values_final_Quest_disk_C_t_energy_eachone_plot_3.csv');
end
...
if (csv_generate_vrr_f == 1)
    ...
    writematrix(C_thr_results_x_vrr_f_range, 'C_thr_results_range_x_vrr_f_final_Quest_disk_C_t_energy_eachone_plot_3.csv');
else
    C_thr_results_x_vrr_f_range_flat = readmatrix('C_thr_results_range_x_vrr_f_final_Quest_disk_C_t_energy_eachone_plot_3.csv');
        ...
        for area_range_i = 1:length(continuous_area_range)
            ...
            for energy_model_index = 1:length(Energy_Model_cell)
                ...
                for csf_model_index = 1:length(CSF_Model_cell)
                    csf_model_use = CSF_Model_cell{csf_model_index};
                    overall_index = (energy_model_index-1)*length(CSF_Model_cell)+csf_model_index;
                    [~,C_thr_results_x_area_range(overall_index,vrr_f_i,area_range_i)] = energy_generate_contrast_all(csf_model_use, energy_model_use, vrr_f_value, area_range_value, radius_range_value, optimized_E_thr_values(overall_index), fit_poly_degree, Luminance_lb, Luminance_ub);
                end
            end
        end
    end
    writematrix(C_thr_results_x_area_range, 'C_thr_results_range_x_area_final_Quest_disk_C_t_energy_eachone_plot_3.csv');
else
    C_thr_results_x_area_range_flat = readmatrix('C_thr_results_range_x_area_final_Quest_disk_C_t_energy_eachone_plot_3.csv');
