classdef CSF_stelaCSF_mod_transient < CSF_base
    ...
    methods
        ...
        function obj = CSF_stelaCSF_mod_transient( )
        ...
        function S = sensitivity( obj, csf_pars )
            ...
            [~, R_trans] = get_sust_trans_resp(obj, omega, ecc);
...
%             end
            ...
            if obj.ps_beta ~= 1 
                beta = obj.ps_beta;
                S = ((R_trans.*S_trans .* sqrt(1./pm_ratio)).^beta + S_aux.^beta).^(1/beta);
            else
                S = R_trans.*S_trans .* sqrt(1./pm_ratio) + S_aux;
            end
    ...
    methods( Static )
        ...
        function p = get_default_par()
            ...
            p.ach_trans.S_max = [ 68.9501 59.5023 0.164274 ];
