...
if plot_vrr_f == 1
    figure;
    ha = tight_subplot(1, 2, [.2 .05],[.23 .06],[.06 .01]);
    ...
    for plot_i = 1:length(suffixes)
        ...
        ylim([0.005, 0.1]);
        if (plot_i == 1)
            ylabel('C_{thr} (Flicker Detection Contrast Threshold)','FontSize',15);
        end
        xlabel('Frequency of Refresh Rate Switch (Hz)','FontSize',15);
        title(suffixes(plot_i),'FontSize',15);
        color = ['r', 'g', 'b', 'm'];
        legend_exp_plots = {};
    ...
    hLegend_2 = legend([legend_exp_plots{1} legend_exp_plots{2} legend_exp_plots{3} legend_exp_plots{4} legend_errorbar_plots{1} ...
                     ...
                     {legend_exp_labels{1} legend_exp_labels{2} legend_exp_labels{3} legend_exp_labels{4} legend_errorbar_labels{1} ...
                      legend_model_labels{1} legend_model_labels{2} legend_model_labels{3} legend_model_labels{4}},'FontSize',10);
    set(hLegend_2, 'Location', 'southoutside', 'Orientation', 'horizontal', 'NumColumns', 3); 
...
if plot_area == 1
    ...
    for plot_i = 1:length(suffixes)
        ...
        xlabel('Area (degree^2)','FontSize',15);
        title(suffixes(plot_i),'FontSize',15);
        color = ['r', 'g', 'b', 'm'];
        legend_exp_plots = {};
        legend_errorbar_plots = {};
        ...
        for vrr_f_i = 1:length(vrr_f_indices)
            ...
            if plot_i == 1
                C_thr_results_x_area_range = C_thr_results_x_area_range_peak;
            else
                C_thr_results_x_area_range = C_thr_results_x_area_range_FF;
            end
