...
for vrr_f_index = 1:length(vrr_f_contiuous)
    ...
    E_stelaCSF_all(vrr_f_index) = energy_model_original(stelaCSF_model, fit_poly_degree, radius, area_value, vrr_f_value, luminance_value);
    E_stelaCSF_HF_all(vrr_f_index) = energy_model_original(stelaCSF_HF_model, fit_poly_degree, radius, area_value, vrr_f_value, luminance_value);
    E_Barten_Original_all(vrr_f_index) = energy_model_original(Barten_Original_model, fit_poly_degree, radius, area_value, vrr_f_value, luminance_value);
    E_Barten_HF_all(vrr_f_index) = energy_model_original(Barten_HF_model, fit_poly_degree, radius, area_value, vrr_f_value, luminance_value);
    E_castleCSF_all(vrr_f_index) = energy_model_original(castleCSF_model, fit_poly_degree, radius, area_value, vrr_f_value, luminance_value);
    E_stelaCSF_transient_all(vrr_f_index) = energy_model_original(stelaCSF_transient_model, fit_poly_degree, radius, area_value, vrr_f_value, luminance_value);
    E_stelaCSF_HF_transient_all(vrr_f_index) = energy_model_original(stelaCSF_HF_transient_model, fit_poly_degree, radius, area_value, vrr_f_value, luminance_value);
end
