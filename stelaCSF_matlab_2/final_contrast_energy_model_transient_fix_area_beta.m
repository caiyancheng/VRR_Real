function [L_thr_stela_transient, L_thr_stela_mod_transient, C_t_stela_transient, C_t_stela_mod_transient] = ...
    final_contrast_energy_model_transient_fix_area_beta(t_frequency, radius, E_thr, fit_poly_degree, fix_area, Luminance_lb, Luminance_ub, beta) 
    ...
    initial_L_thr = 1;
    integrand_stela = @(rho, L_thr) 2 * pi * (2 * pi * radius)^(1/beta) *get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_transient(rho, L_thr, fix_area, t_frequency)).^2) .* rho;
    integrand_stela_mod = @(rho, L_thr) 2 * pi * (2 * pi * radius)^(1/beta) *get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_mod_transient(rho, L_thr, fix_area, t_frequency)).^2) .* rho;
...
function [L_thr_stela_transient, C_t_stela_transient] = final_contrast_energy_model_stela_transient_fix_area(t_frequency, radius, E_thr, fit_poly_degree, fix_area) 
    delta_rho = 0.01;  % Width of each small interval
    num_rho_points = 5000;
    rho_sum = linspace(0, num_rho_points * delta_rho, num_rho_points)';  % Transpose to make it a column vector
    integrand_stela = @(rho, L_thr) 2 * pi * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_transient(rho, L_thr, fix_area, t_frequency)).^2) .* rho;
    E_stela = @(L_thr) delta_rho * sum(integrand_stela(rho_sum, L_thr));
    lb = 0.04;
    ub = 10;
...
function [L_thr_stela_mod_transient, C_t_stela_mod_transient] = final_contrast_energy_model_stela_mod_transient_fix_area(t_frequency, radius, E_thr, fit_poly_degree, fix_area) 
    delta_rho = 0.01;  % Width of each small interval
    num_rho_points = 5000;
    rho_sum = linspace(0, num_rho_points * delta_rho, num_rho_points)';  % Transpose to make it a column vector
    integrand_stela_mod = @(rho, L_thr) 2 * pi * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_mod_transient(rho, L_thr, fix_area, t_frequency)).^2) .* rho;
    E_stela_mod = @(L_thr) delta_rho * sum(integrand_stela_mod(rho_sum, L_thr));
    lb = 0.04;
    ub = 10;
    options = optimset('Display', 'off');
