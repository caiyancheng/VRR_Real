...
suffixes = {'BartenCSF (Multiple Contrast Energy Detectors)', 'BartenCSF_{HF} (Multiple Contrast Energy Detectors)', 'castleCSF (Multiple Contrast Energy Detectors)', 'castleCSF (Spatial Contrast Energy Model, p=3.5)', 'castleCSF (Spatial Contrast Energy Model + Eccentricity, p=3.6)'};
...
CSF_Model_cell = {Barten_Original_model, Barten_HF_model, castleCSF_model, castleCSF_model, castleCSF_model};
...
initial_E_thr_values(1) = 1.67307536856838;
initial_E_thr_values(2) = 0.265328780340484;
initial_E_thr_values(3) = 0.267551350144257;
initial_E_thr_values(4) = 111218.924428437;
initial_E_thr_values(5) = 137582.1334;
...
if (optimize_need == 1)
    if length(skip_optimize_list) > 0
        optimized_E_thr_values_csv = readmatrix('optimized_E_thr_values_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
        optimized_fvals_csv = readmatrix('fvals_x_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
    end
    ...
    writematrix(optimized_E_thr_values, 'optimized_E_thr_values_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
    writematrix(fvals, 'fvals_x_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
else
    optimized_E_thr_values = readmatrix('optimized_E_thr_values_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
end
...
if (csv_generate_vrr_f == 1)
    ...
    writematrix(C_thr_results_x_vrr_f_range, 'C_thr_results_range_x_vrr_f_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
else
    C_thr_results_x_vrr_f_range_flat = readmatrix('C_thr_results_range_x_vrr_f_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
...
if (csv_generate_area == 1)
    ...
    writematrix(C_thr_results_x_area_range, 'C_thr_results_range_x_area_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
else
    C_thr_results_x_area_range_flat = readmatrix('C_thr_results_range_x_area_final_Quest_disk_C_t_energy_eachone_plot_8.csv');
...
if plot_vrr_f == 1
    figure;
    ha = tight_subplot(1, 5, [.07 .02],[.23 .05],[.035 .01]);
    ...
    for plot_i = 1:length(suffixes)
        ...
        for size_i = 1:length(size_indices)
            ...
            set(gca, 'XScale', 'log');
            set(gca, 'YScale', 'log');
            legend_exp_plots{end+1} = plot(vrr_f_indices, average_C_t_matrix(:, size_i), 'o-', 'Color', color(size_i), 'MarkerFaceColor', color(size_i), 'LineWidth', 1.0, 'DisplayName', display_name_exp);
...
if plot_area == 1
    ...
    for plot_i = 1:length(suffixes)
        ...
        for vrr_f_i = 1:length(vrr_f_indices)
            ...
            set(gca, 'XScale', 'log');
            set(gca, 'YScale', 'log');
            legend_exp_plots{end+1} = plot(area_indices, average_C_t_matrix(vrr_f_i,:), 'o-', 'Color', color(vrr_f_i), 'MarkerFaceColor', color(vrr_f_i), 'LineWidth', 1.0, 'DisplayName', display_name_exp);
            legend_exp_labels{end+1} = display_name_exp;
            if (vrr_f_i == 1)
