...
suffixes = {'stelaCSF (FF)', 'stelaCSF_{HF} (FF)', 'BartenCSF (FF)', 'BartenCSF_{HF} (FF)', ...
            'castleCSF (FF)', 'stelaCSF transient (FF)', 'stelaCSF_{HF} transient (FF)'};
...
if optimize_need == 1
    ...
    for csf_model_index = 1:length(CSF_Model_cell)
        ...
        S_FF = S_FF_generate(csf_model_use, initial_setting_luminance_value, initial_setting_area_value, initial_setting_vrr_f_value);
        C_thr_sub = average_C_t_matrix(initial_setting_vrr_f_index, initial_setting_radius_index);
        initial_k_scale_values(csf_model_index) = 1 /  (S_FF * C_thr_sub);
    end
end
...
if (optimize_need == 1)
    ...
    for csf_model_index = 1:length(CSF_Model_cell)
        csf_model_use = CSF_Model_cell{csf_model_index};
        objective_function = @(k_scale_value) FF_fit_loss_all(csf_model_use, size_indices, vrr_f_indices, average_C_t_matrix, ...
            k_scale_value, fit_poly_degree, Luminance_lb, Luminance_ub).*loss_multiple_factor;
    ...
    writematrix(optimized_k_scale_values, 'optimized_k_scale_values_final_Quest_disk_C_t_FF_eachone_plot_1.csv');
    writematrix(fvals, 'fvals_final_Quest_disk_C_t_FF_eachone_plot_1.csv');
else
    optimized_k_scale_values = readmatrix('optimized_k_scale_values_final_Quest_disk_C_t_FF_eachone_plot_1.csv');
end
        ...
        for vrr_f_range_i = 1:length(continuous_vrr_f_range)
            ...
            for csf_model_index = 1:length(CSF_Model_cell)
                ...
                [~,C_thr_results_x_vrr_f_range(csf_model_index,vrr_f_range_i,size_i)] = ...
                    FF_generate_contrast_all(csf_model_use, vrr_f_range_value, area_value, radius, ...
                    optimized_k_scale_values(csf_model_index), fit_poly_degree, Luminance_lb, Luminance_ub);
            end
        end
    end
    writematrix(C_thr_results_x_vrr_f_range, 'C_thr_results_range_x_vrr_f_final_Quest_disk_C_t_FF_eachone_plot_1.csv');
else
    C_thr_results_x_vrr_f_range_flat = readmatrix('C_thr_results_range_x_vrr_f_final_Quest_disk_C_t_FF_eachone_plot_1.csv');
        ...
        for area_range_i = 1:length(continuous_area_range)
            ...
            for csf_model_index = 1:length(CSF_Model_cell)
                ...
                [~,C_thr_results_x_area_range(csf_model_index,vrr_f_i,area_range_i)] = ...
                    FF_generate_contrast_all(csf_model_use, vrr_f_value, area_range_value, radius, ...
                    optimized_k_scale_values(csf_model_index), fit_poly_degree, Luminance_lb, Luminance_ub);
            end
        end
    end
    writematrix(C_thr_results_x_area_range, 'C_thr_results_range_x_area_final_Quest_disk_C_t_FF_eachone_plot_1.csv');
else
    C_thr_results_x_area_range_flat = readmatrix('C_thr_results_range_x_area_final_Quest_disk_C_t_FF_eachone_plot_1.csv');
