...
for vrr_f_index = 1:length(VRR_F_list)
    ...
    for size_index = 1:length(Size_list)
        ...
        plot(L_plot_array, fit_contrast, 'Color', colors{vrr_f_index}, 'LineWidth', 2, 'DisplayName', 'linear fitting result');
        index = index + 1;
    end
    legend(hh, 'Location', 'northeast', 'Orientation', 'horizontal', 'NumColumns', 1, FontSize=12);
