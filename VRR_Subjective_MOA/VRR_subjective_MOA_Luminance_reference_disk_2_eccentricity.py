...
def vrr_one_block_disk(glfw, window, vrr_params, c_params):
    ...
    while not glfw.window_should_close(window):
        ...
        glColor3f(initial_color * 1.5, initial_color * 1.5, initial_color * 1.5)
        glBegin(GL_QUADS)
    ...
    while not glfw.window_should_close(window):
        ...
        glColor3f(color * 1.5, color * 1.5, color * 1.5)
        glBegin(GL_QUADS)
        glVertex2f(0 - center_point_size_x / 2, 0 - center_point_size_y / 2)
        glVertex2f(0 + center_point_size_x / 2, 0 - center_point_size_y / 2)
        glVertex2f(0 + center_point_size_x / 2, 0 + center_point_size_y / 2)
...
def vrr_exp_main(change_parameters, vrr_params, save_path, random_shuffle, continue_exp):
    ...
    for re_i in range(change_parameters['Repeat_times']):
        ...
        while index < len(setting_list):
            ...
            if continue_exp and f'V_{vrr_f}_S_{size}_E_{ecc}' in experiment_json_dict.keys():
                print('Jump')
                index = index + 1
                continue
            experiment_json_dict[f'V_{vrr_f}_S_{size}_E_{ecc}'] = []
...
if __name__ == "__main__":
    ...
    save_base_path = r'../VRR_Subjective_MOA/Result_MOA_disk_eccentricity_1/'
