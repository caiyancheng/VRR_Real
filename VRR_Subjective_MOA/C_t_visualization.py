...
for size_index in range(len(MOA_Sizes)):
    ...
    for vrr_f_index in range(len(MOA_VRR_Fs)):
        ...
        C_t_result = MOA_C_t_result[f'V_{vrr_f_value}_S_{size_value}']['C_t']
        x_vrr_f.append(vrr_f_value)
        y_C_t.append(C_t_result)
    plt.plot(x_vrr_f, y_C_t, label=f'Size_{size_value}')
    plt.scatter(x_vrr_f, y_C_t, marker='o')
plt.legend()
plt.title(f'Observer_{Observer_Name} MOA Experiment Result')
plt.xlabel('Frequency of RR Switch (Hz)')
plt.ylabel('C_t - Detection Threshold')
plt.show()
