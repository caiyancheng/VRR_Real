...
def vrr_one_block_disk(glfw, window, vrr_params, c_params):
    ...
    while not glfw.window_should_close(window):
        ...
        for i in range(num_segments + 1):
            ...
            distance_to_edge = math.sqrt((x - x_center) ** 2 + ((y - y_center)/y_scale*x_scale) ** 2)
            blur_factor = lanczos_filter(distance_to_edge / blur_radius)
            glColor3f(initial_color * blur_factor, initial_color * blur_factor, initial_color * blur_factor)
            glVertex2f(x, y)
...
if __name__ == "__main__":
    ...
    save_base_path = r'../VRR_Subjective_MOA/Result_MOA_disk_blur_edge_1/'
