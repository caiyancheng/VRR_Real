...
def temporal_record_main(change_parameters, vrr_params, color_change_parameters, record_params, real_save_path, random_shuffle):
    ...
    glfw.set_input_mode(window, glfw.CURSOR, glfw.CURSOR_DISABLED)
    global sensor
    sensor = temporal_light_sensor.TemporalLightSensor(serial.Serial("COM4", 500000, timeout=record_params['time_maximum']))
...
if __name__ == "__main__": #必须要用多线程，否则会出现问题
    change_parameters = {
        'VRR_Frequency': [0.5, 2, 4, 8, 10, 12, 14, 16, 20],
        'Size': [0.5, 1, 16, 'full'],
        'Repeat_times': 2, #每条应该记录2遍
    }
    ...
    save_path = r'B:\Datasets\Temporal_Flicker_Meter_log_Krypton/deltaL_L_10second_9VRR_4Size_2repeat_30color_log10'
