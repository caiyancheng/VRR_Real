...
subfigure_top = fig.add_subplot(gs[0, 0])
subfigure_middle = fig.add_subplot(gs[1, 0])

for plot_index in range(len(x_Time_array_plot_list)):
    plot_index = len(x_Time_array_plot_list) - plot_index - 1
    subfigure_top.plot(x_Time_array_plot_list[plot_index], y_Luminance_array_plot_list[plot_index], linewidth=0.8)
# plt.xlabel('Time (s)', fontsize=14)
subfigure_top.set_ylabel('Luminance (cd/m$^2$)', fontsize=14)
subfigure_top.set_yscale('log')
subfigure_top.set_ylim([-1,1])
subfigure_top.set_yticks([0.1, 1, 10],['0.1', '1', '10'])
subfigure_top.set_title('30Hz-120Hz VRR ($F_{rrs}$ ='+f'{aim_vrr_f}Hz)')
# plt.grid(True)
...
subfigure_middle.plot(x_time, y)
subfigure_middle.set_xlabel('Time (s)', fontsize=14)
subfigure_middle.set_ylabel('Refresh Rate (Hz)', fontsize=14)
subfigure_middle.set_yticks([30,120])
# plt.show()
...
ax.grid(True)
ax.legend()
ax.view_init(elev=10, azim=-70)  # 调整视角
...
plt.subplots_adjust(left=0.1, right=0.99, top=0.95, bottom=0.0)
plt.show()
