...
for plot_index in range(len(x_Time_array_plot_list)):
    ...
    axs[0].plot(x_Time_array_plot_list[plot_index], y_Luminance_array_plot_list[plot_index], linewidth=0.8)
# plt.xlabel('Time (s)', fontsize=14)
axs[0].set_ylabel('Luminance (cd/m$^2$)', fontsize=14)
axs[0].set_yscale('log')
axs[0].set_ylim([-1,1])
axs[0].set_yticks([0.1, 1, 10],['0.1', '1', '10'])
axs[0].set_title('30Hz-120Hz VRR ($F_{rrs}$ ='+f'{aim_vrr_f}Hz)')
# plt.grid(True)
...
axs[1].set_ylabel('Refresh Rate (Hz)', fontsize=14)
axs[1].set_yticks([30,120])
# plt.show()
...
# plt.show()
plt.savefig(f'E:\All_Conference_Papers\SIGGRAPH24\Images_new/final_FFT_{aim_vrr_f}_3d_label_2.pdf', format='pdf')