...
for luminance_index in range(len(luminance_values)):
    for obs_index in range(len(obs_list)):
        ...
        sub_df = df_dict[obs][df_dict[obs]['Luminance'] == luminance_values[luminance_index]]
        accuracy = (sub_df['Real_VRR_period'] == sub_df['Observer_choice']).mean()
        accuracy_array[luminance_index][obs_index] = accuracy

with open(r'E:\Py_codes\VRR_Real\Computational_Model/C_2_array.json', 'r') as fp:
    C_2_array = np.array(json.load(fp))

P_CSF_luminance = np.zeros(len(luminance_values))
for luminance_i in range(len(luminance_values)):
    sub_C_2_array = C_2_array[luminance_i,:,:]
    C_2_array_luminance_flat = sub_C_2_array.reshape(-1)
    P_CSF_luminance_i = compute_P_from_C_T(C_T=C_2_array_luminance_flat, mu=0.003937395993406365)
    P_CSF_luminance[luminance_i] = np.mean(P_CSF_luminance_i)
...
plt.bar(np.log10(luminance_values), mean_array, width=bar_width, label='Observers Real', color='lightgreen', edgecolor='green', alpha=1)
plt.bar(np.log10(luminance_values) + bar_width, P_CSF_luminance, width=bar_width, label='StelaCSF Prediction', color='royalblue', edgecolor='blue', alpha=1)
plt.errorbar(np.log10(luminance_values), mean_array, yerr=std_dev_array,
             fmt='none', color='red', capsize=3, label='Standard Deviation across Observers')
plt.errorbar(np.log10(luminance_values), mean_array, yerr=[mean_array - bino_error_bar[:, 0], bino_error_bar[:, 1] - mean_array],
