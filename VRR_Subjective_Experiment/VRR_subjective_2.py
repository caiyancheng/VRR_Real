...
center_point_size_x = 0.005  # Adjust the size of the center white point as needed
...
def vrr_exp_main(change_parameters, vrr_params, signal_params, save_path):
    with open('E:\Py_codes\VRR_Real\G1_Calibration\Official_Config\Aim_L_[1, 10, 100]_Size_[4, 16]/result_dict.json', 'r') as fp:
        luminance_dict = json.load(fp)
    experiment_record = {'VRR_Frequency': [], 'Luminance': [], 'Size(Degree)': [], 'Eccentricity': [],
            ...
            for size in change_parameters['Size']:
                for ecc in change_parameters['Eccentricity']:
                    for re_i in range(change_parameters['Repeat_times']):
                        ...
                        experiment_record['Size(Degree)'].append(size)
...
if __name__ == "__main__":
    ...
    change_parameters = {
        ...
        'Size': [4,16],
        'Eccentricity': [0,15],
        'Repeat_times': 1, #同一个setting重复多少遍
    }
    vrr_params = {
        'frame_rate_min': 30,
        'frame_rate_max': 120,
    ...
    observer_params = {
        'name': 'Yancheng_Cai_Test',
        'age': 20,
        'gender': 'M',
    }
    print(change_parameters)
    save_base_path = r'E:\Py_codes\VRR_Real\VRR_Subjective_Experiment/Result_VRR_2/'
