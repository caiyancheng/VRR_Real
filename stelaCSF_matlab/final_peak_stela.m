function [L_thr_stela, C_t_stela] = final_peak_stela(t_frequency, area_value, radius, k_scale, fit_poly_degree, Luminance_lb, Luminance_ub, peak_spatial_frequency)
    initial_L_thr = 3;
    options = optimset('Display', 'off');
    fun_stela = @(L_thr) abs(get_contrast_from_Luminance(L_thr, fit_poly_degree, radius) - ...
        1 / (k_scale*max(S_stela(peak_spatial_frequency, L_thr, area_value, t_frequency))));
