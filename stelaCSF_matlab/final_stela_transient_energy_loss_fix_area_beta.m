function loss_sum = final_stela_transient_energy_loss_fix_area_beta(size_indices, vrr_f_indices, average_C_t_matrix, E_thr_value, fit_poly_degree, area_fix, Luminance_lb, Luminance_ub, beta)
    ...
    for size_i = 1:length(size_indices)
        ...
        for vrr_f_i = 1:length(vrr_f_indices)
            ...
            [~,C_t_predict_results_energy_fit(vrr_f_i, size_i)] = final_contrast_energy_model_stela(vrr_f_value, area_fix, radius, E_thr_value, fit_poly_degree, Luminance_lb, Luminance_ub, beta);
...
function [L_thr_stela, C_t_stela] = final_contrast_energy_model_stela(t_frequency, area_value, radius, E_thr, fit_poly_degree, Luminance_lb, Luminance_ub, beta) 
    ...
    initial_L_thr = 3;
    integrand_stela = @(rho, L_thr) 2 * pi * (2 * pi * radius)^(1/beta) * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_transient(rho, L_thr, area_value, t_frequency)).^2) .* rho;
