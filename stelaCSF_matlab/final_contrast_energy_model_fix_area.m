function [L_thr_stela, L_thr_stela_mod, L_thr_barten_mod, C_t_stela, C_t_stela_mod, C_t_barten_mod] = ...
    final_contrast_energy_model_fix_area(t_frequency, radius, E_thr, fit_poly_degree, fix_area) 
    ...
    initial_L_thr = 3;
    initial_E = integral(@(rho) 2 * pi * get_contrast_from_Luminance(initial_L_thr, fit_poly_degree, radius)^2 * ((D(rho, r) .* S_stela(rho, L_thr, fix_area, t_frequency)).^2) .* rho, 0, inf);
    lb = 0.04;
    ub = 10;
    integrand_stela = @(L_thr) integral(@(rho) 2 * pi * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela(rho, L_thr, fix_area, t_frequency)).^2) .* rho, 0, inf);
    integrand_stela_mod = @(L_thr) integral(@(rho) 2 * pi * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_mod(rho, L_thr, fix_area, t_frequency)).^2) .* rho, 0, inf);
    integrand_barten_mod = @(L_thr) integral(@(rho) 2 * pi * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_barten_mod(rho, L_thr, fix_area, t_frequency)).^2) .* rho, 0, inf);
