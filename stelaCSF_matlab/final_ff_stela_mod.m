function [L_thr_stela_mod, C_t_stela_mod] = final_ff_stela_mod(t_frequency, area_value, radius, k_scale, fit_poly_degree, Luminance_lb, Luminance_ub)
    initial_L_thr = 3;
    rho = 1/(2*(pi)^0.5*radius);
    options = optimset('Display', 'off');
    fun_stela_mod = @(L_thr) abs(get_contrast_from_Luminance(L_thr, fit_poly_degree, radius) - 1 / (k_scale*S_stela_mod(rho, L_thr, area_value, t_frequency)));
