function [L_thr_stela, L_thr_stela_mod, C_t_stela, C_t_stela_mod] = final_contrast_energy_model_transient(t_frequency, area_value, radius, E_thr, fit_poly_degree) 
    ...
    initial_L_thr = 3;
    initial_E = integral(@(rho) 2 * pi * get_contrast_from_Luminance(initial_L_thr, fit_poly_degree, radius)^2 * ((D(rho, r) .* S_stela_transient(rho, L_thr, area_value, t_frequency)).^2) .* rho, 0, inf);
    lb = 0.04;
    ub = 10;
    integrand_stela = @(L_thr) integral(@(rho) 2 * pi * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_transient(rho, L_thr, area_value, t_frequency)).^2) .* rho, 0, inf);
    integrand_stela_mod = @(L_thr) integral(@(rho) 2 * pi * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_mod_transient(rho, L_thr, area_value, t_frequency)).^2) .* rho, 0, inf);
