function loss_sum = final_barten_mod_peak_loss(size_indices, vrr_f_indices, average_C_t_matrix, k_scale, fit_poly_degree, Luminance_lb, Luminance_ub, peak_spatial_frequency)
    ...
    for size_i = 1:length(size_indices)
        ...
        for vrr_f_i = 1:length(vrr_f_indices)
            ...
            [~,C_t_predict_results_energy_fit(vrr_f_i, size_i)] = final_peak_barten_mod(vrr_f_value, area_value, radius, k_scale, fit_poly_degree, Luminance_lb, Luminance_ub, peak_spatial_frequency);
