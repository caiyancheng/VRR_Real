...
initial_k_values = [2, 2, 2, 1000, 1000, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]';
...
for size_value = size_indices
    for luminance_value = luminance_indices
        ...
        plot(vrr_f_CSF_list, C_t_s(4,:), 'c-', 'LineWidth', 3, 'DisplayName', sprintf('stelaCSF transient (Energy) - k: %.4f, loss (*1e8): %.4f', optimized_k_values(4), fvals(4)));
        plot(vrr_f_CSF_list, C_t_s(5,:), 'm-', 'LineWidth', 3, 'DisplayName', sprintf('stelaCSF_{mod} transient (Energy) - k: %.4f, loss (*1e8): %.4f', optimized_k_values(5), fvals(5)));
        ...
        plot(vrr_f_CSF_list, C_t_s(15,:), 'Color', [0.3, 0.6, 0.3], 'LineWidth', 3, 'DisplayName', sprintf('stelaCSF transient (Peak) - k: %.4f, loss (*1e8): %.4f', optimized_k_values(15), fvals(15)));
        plot(vrr_f_CSF_list, C_t_s(16,:), 'Color', [0.9, 0.7, 0.1], 'LineWidth', 3, 'DisplayName', sprintf('stelaCSF_{mod} transient (Peak) - k: %.4f, loss (*1e8): %.4f', optimized_k_values(16), fvals(16)));
