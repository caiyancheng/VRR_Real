function [result_stela, result_stela_mod, result_barten_mod] = multiple_contrast_energy_detectors_cyc_1(L_b, c, beta, num_points, t_frequency, size_value) 
    ...
    r = size_value/2;
    integrand_stela = @(rho) (c^2 * (2 * pi * r).^(1 / beta) .* (D(rho, r) .* S_stela(rho, L_b, size_value, t_frequency)).^2) .* rho;
    integrand_stela_mod = @(rho) (c^2 * (2 * pi * r).^(1 / beta) .* (D(rho, r) .* S_stela_mod(rho, L_b, size_value, t_frequency)).^2) .* rho;
    integrand_barten_mod = @(rho) (c^2 * (2 * pi * r).^(1 / beta) .* (D(rho, r) .* S_barten_mod(rho, L_b, size_value, t_frequency)).^2) .* rho;
    ...
    result_stela = result_stela * delta_rho * t_frequency.^2;
    result_stela_mod = result_stela_mod * delta_rho * t_frequency.^2;
    result_barten_mod = result_barten_mod * delta_rho * t_frequency.^2;
end
