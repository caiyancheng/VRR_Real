function loss_sum = final_stela_mod_energy_loss_fix_area_beta(size_indices, vrr_f_indices, average_C_t_matrix, E_thr_value, fit_poly_degree, area_fix, Luminance_lb, Luminance_ub, beta)
...
function [L_thr_stela_mod, C_t_stela_mod] = final_contrast_energy_model_stela_mod(t_frequency, area_value, radius, E_thr, fit_poly_degree, Luminance_lb, Luminance_ub, beta) 
    ...
    initial_L_thr = 3;
    integrand_stela_mod = @(rho, L_thr) 2 * pi * (2 * pi * radius)^(1/beta) * get_contrast_from_Luminance(L_thr, fit_poly_degree, radius)^2 * ((D(rho, radius) .* S_stela_mod(rho, L_thr, area_value, t_frequency)).^2) .* rho;
